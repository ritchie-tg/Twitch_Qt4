...
def Livestreamer(streamName, oauth, cache, quality):
	...
	#print 'Attempting to open stream: {0}'.format(streamName)
...
def GetImage(address):
	...
	assetsPath = os.path.dirname(os.path.realpath(__file__)) + '/img_cache/' #for images
	fileName = address.split("/")[-1]
	for img in os.listdir(assetsPath):
		if img == fileName:
			gate = 1
			break
	...
	if not gate:
		try:
			urllib.urlretrieve(address, assetsPath+fileName)
			return True
